...
test.describe('FundHoldings', () => {
  ...
  test('all holdings are loaded into the table', async ({ page }) => {
    ...
    for (const h of holdings) {
      await expect(page.getByRole('cell', { name: h.ticker, exact: true })).toBeVisible()
    }
  })

  // ─── Search filters ───────────────────────────────────────────────────────────

  test('filters by ticker substring (case-insensitive) on Apply', async ({ page }) => {
  ...
  test('filters by name substring (case-insensitive) on Apply', async ({ page }) => {
  ...
  test('filters by max weight on Apply', async ({ page }) => {
    ...
    await expect(page.getByRole('cell', { name: 'GOOGL', exact: true })).toBeVisible()
  })
  ...
  test('clicking a column header sorts the table ascending', async ({ page }) => {
  ...
  test('clicking the same column header again sorts descending', async ({ page }) => {
  ...
  test('clicking a different column clears sort icons from the previous column', async ({ page }) => {
  ...
  test('clicking the ✕ icon clears the sort and restores original order', async ({ page }) => {
    const tickerHeader = page.getByRole('columnheader', { name: /Ticker/ })
    await tickerHeader.click()
    await expect(page.locator('tbody tr').first().locator('td').first()).toHaveText('AAPL')

    await page.getByTitle('Clear sort').click()

    // Original fixture order: AAPL, MSFT, NVDA, AMZN, GOOGL
    const tickers = await page.locator('tbody tr td:first-child').allTextContents()
